...
/**
 * Strip any AI/assistant attribution from a commit message.
 *
 * Guarantees commits carry NO trace of the assistant — removes Claude/AI
 * co-author trailers, "Generated with Claude Code" lines, robot emoji
 * trailers, and similar markers. Whatever the source of the message
 * (orchestrator-built or agent-supplied), the persisted commit is clean.
 */
export function sanitizeCommitMessage(message: string): string {
	const lines = message.split(/\r?\n/);
	const kept = lines.filter(line => {
		const l = line.trim();
		if (!l) return true; // keep blank lines (collapsed later)
		// Co-author trailers pointing at the assistant / Anthropic
		if (/^co-authored-by:.*(claude|anthropic|noreply@anthropic)/i.test(l)) return false;
		// "Generated with Claude Code" / "Generated by Claude" promo lines
		if (/generated (with|by)\s+claude/i.test(l)) return false;
		if (/🤖\s*generated/i.test(l)) return false;
		// Standalone robot-emoji promo line
		if (/^🤖/.test(l)) return false;
		// "as an AI" / assistant self-reference trailers
		if (/^(this commit was )?(created|written|authored|made) (with|by) (claude|an? ai|the assistant)/i.test(l)) return false;
		return true;
	});
...
export function atomicCommit(cwd: string, taskId: string, message: string, taskFiles?: string[]): boolean {
	try {
		...
		const commitMsg = sanitizeCommitMessage(`${taskId}: ${message}`);
...
export interface SafePushOptions {
	/** Branches that require explicit opt-in to push to. Default: main, master. */
	protectedBranches?: string[];
	/** Allow pushing even when on a protected branch. Default false. */
...
/**
 * Push the CURRENT branch to its remote — safely.
 *
 * Guarantees, by construction:
 *   - NEVER force-pushes (no --force / --force-with-lease).
 *   - NEVER pushes anything but the checked-out branch (no --all, no other refs).
 *   - Refuses a detached HEAD (no branch to push).
 *   - Refuses protected branches (main/master) unless `allowProtected` is set.
 *   - Sets upstream with `-u origin <branch>` on first push.
 *
 * Returns a structured result instead of throwing so callers can log & continue.
 */
export function safePush(cwd: string, options?: SafePushOptions): SafePushResult {
	...
	try {
		...
		if (!branch || branch === "HEAD") {
			log.warn("git", "Push skipped: detached HEAD (no branch to push)");
			return { pushed: false, reason: "detached HEAD" };
		}

		if (protectedBranches.includes(branch) && !options?.allowProtected) {
			log.warn("git", `Push skipped: refusing to auto-push protected branch "${branch}" (pass allowProtected to override)`);
		...
		try {
			execFileSync("git", ["rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{u}"], { cwd, stdio: "pipe" });
		} catch {
			hasUpstream = false;
		}

		const args = hasUpstream ? ["push"] : ["push", "-u", "origin", branch];
		execFileSync("git", args, { cwd, stdio: "pipe", timeout: 60000 });
		log.info("git", `Pushed "${branch}" to remote${hasUpstream ? "" : " (set upstream)"}`);
		return { pushed: true, branch };
	} catch (e: any) {
		const msg = e?.stderr?.toString?.() || e?.message || String(e);
		log.warn("git", `Push failed: ${String(msg).slice(0, 200)}`);
		return { pushed: false, reason: "push command failed" };
	}
}
